...
>已经建立TCP连接的两台主机, 都为该连接设置了接收缓存. 收到按序达到的数据后, TCP就将这些数据暂存到接收缓存, 相应的应用程序会从接收缓存中读取数据. 
>然而, 应用程序并不一定能够立刻将接收缓存中的数据取走, 这可能是因为程序正忙于其他任务, 需要经过较长的时间才能取走. 

如果应用程序从接收缓存中取走数据比较慢, 而发送方持续快速发送大量数据, 则容易造成接收方的接收缓存溢出, 也就是造成数据损失.
...
- 举例说明:
	...
	- 在建立连接时, B告诉A自己的接收窗口(Receive Window) rwnd是400, A就把自己的发送窗口(Send Windows) swnd 也设置成400. 这就意味着, A接收到B发来的ACK之前, 可以将落入发送窗口的数据都发送出去.
		- ![](assets/Pasted%20image%2020230419115931.png)
	- 假设每个报文段发送100字节, 发送窗口内还有300个字节可以发送. seq表示序号字段, DATA表示这是TCP数据报文段.
		- ![](assets/Pasted%20image%2020230419120258.png)
	- seq = 201的数据报文段丢失了, 窗口内还有最后100字节的内容可以发. 此时, B给A发送确认报文段, 确认201前面的报文段已经被接收, 同时告诉A自己的接收窗口rwnd改成300了. 可以简单认为B的接收缓存可以空间为300字节, 对主机A进行流量控制. A就把自己的发送窗口从400改为300. 现在窗口落入201到500. 201到300还没被确认, 不能删, 可能还要超时重传. 301到400, 401到500 都发送出去了.
		- ![](assets/Pasted%20image%2020230419115534.png)
	- 假设此时A发送窗口内的201到300数据的重传定时器超时了, A超时重传. 此时A暂时不能发送其他数据.
	- B收到重传的seq= 201, 给A发送确认报文段, 确认收到201~500这段. 同时再次修改自己的接收窗口rwnd = 100.
	- 发送完seq=501的数据报文段后, B又给A发送普通确认报文段, 这次把rwnd改为0了. 第三次流量控制. A收到确认报文段, 可以把已发送的数据移出窗口, 删除发送缓存. 然后A把自己的发送窗口改为0, 不能再发送数据. 
		...
		- 持续计时器的和零窗口探测报文段的作用:
			- A收到零窗口通知, 启动持续计时器. 非零窗口通知丢失, 持续计时器超时, A发送零窗口探测报文段. B收到探测报文段后给A发送确认, 并告知A自己的接收窗口.若窗口依旧是0, 那重复上面. 若窗口非零, 打破死锁局面.
...
- 拥塞控制与流量控制的区别：
	...
	- 表面上看, Flow Control 与 Congestion Control非常相似, 因为它们都需要控制源点的发送速率. 但实际上有很大差别: 
		...
		- 拥塞控制的任务是, 防止过多的数据注入到网络中, 使网络能够承受先有的网络负荷. 这是一个全局性的问题, 涉及各方面的行为, 包括网络中的所有主机, 路由器, 路由器内部的存储转发处理过程, 还有与降低网络传输性能有关的所有因素.
...
- 拥塞控制的方法：
	...
	- 注意: 拥塞控制不仅仅是运输层要考虑的问题. 显示反馈算法必须涉及到网络层.  虽然一些网络体系结构(如ATM网络) 主要在网络层实现拥塞控制, 但因特网主要利用 隐式反馈在运输层实现拥塞控制
...
- 一些前置条件: 
	- ![](assets/Pasted%20image%2020230419145248.png)
		- 发送方维护两个窗口, 发送和拥塞两个窗口, 而接收方 维护一个接收窗口.
		- 拥塞窗口cwnd的维护原则: 只要网络没有出现拥塞, cwnd就增大一些, 但只要网络出现拥塞, cwnd就减少一些.
		- ![](assets/Pasted%20image%2020230419150055.png)
		- 判断网络拥塞的依据: 没有按时收到应该到达的确认报文段而产生了超时重传.(因为现在通信线路传输质量一般都很好, 因误码而被丢失分组的概率小于1%, 所以当出现超时重传, 大概率是网络中某个路由器繁忙, 丢弃的. 这是网络出现拥塞的征兆).
		- 除了cwnd和swnd, 发送方还需要维护一个叫慢开始门限(SS thresh)的状态变量, ssthresh.

- 慢开始算法和拥塞避免算法(举例说明)
	- （1988年Tahoe版）
	- 在TCP双方建立连接时, 拥塞窗口cwnd的初始值置为1(因为主机刚开始发送数据时完全不知道网络的拥塞情况, 若立即把大量数据注入网络, 可能引起网络拥塞). 经验证明, 较好的方法是由小到大逐渐增大发送方的cwnd的数值, 直到拥塞发生. 
	- 另外还需要设置慢开始门限ssthresh的初始值, 本例为16.
	- 在执行慢开始算法时, 发送方没收到一个对新报文段的确认时, 就把拥塞窗口cwnd的值加1, 然后开始下一轮的传输. 当拥塞窗口cwnd的值增长到慢开始门限ssthesh的值时, 就改为执行避免拥塞算法.

	- (1) 发送方给接收方发送0号数据报文段. 接收方收到后, 给发送发发回对0号报文段的确认报文段. 发送方接收到确认报文段后, 将cwnd的值增加到2. 这意味着下一次传输轮次中发送方可以发送两个报文段了. (传输轮次市值发送方发送数据报文段后, 接收方给发送方回确认报文段. 一个传输轮次经历的时间就是往返时间RTT, RTT并非恒定值)
		- ![](assets/Pasted%20image%2020230419150647.png)
		- (2) (3) 次传输轮次使得cwnd为8了.
	- (4) 本次传输轮次, 使得cwnd增大到16 与ssthresh相等. 因此开始改用避免拥塞算法. 也就是每个传输轮次结束后, cwnd的值只能线性加1了.
		- ![](assets/Pasted%20image%2020230419152335.png)
	- (8) 经过(5)到(7)的拥塞避免算法, cwnd到了24. 假设在发送171到194号数据报文段出现了丢失的情况. 这必然会导致发送方的超时重传. 发送方以此判断出现了网络拥塞, 需要调整自己的cwnd和ssthresh的值:
		- ssthresh = cwnd/2 = 12
		- cwnd = 1. 并且重新执行慢开始算法.
...
>3. 【超时】，ssthresh变为当前cwnd的一半（不能<2）；
>4. 再从【慢开始】，拥塞窗口从1指数增长。

- 快重传算法和快恢复算法（改进TCP性能，1990年Reno版）
	- 改进TCP的性能
	- 有时候个别TCP报文段会在网络中丢失(如误码丢弃), 但实际上网络并未发生拥塞. 这导致发送方超时重传, 并误以为网络发生了拥塞. 然后又回到慢开始算法, 这就是因为误解而导致的效率低下.
	- ![](assets/Pasted%20image%2020230419154202.png)
	- ![](assets/Pasted%20image%2020230419154209.png)
- 快重传算法:
	- 采用快重传算法可以让发送方`尽早知道`发生了个别TCP报文段的丢失。
...
- 快恢复算法
	- 与快重传算法配合使用的是快恢复算法，发送方一旦`收到3个重复确认`，就知道现在只是丢失了个别的报文段，于是不启动慢开始算法，而是`执行快恢复算法`。
